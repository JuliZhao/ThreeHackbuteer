...
@property (nonatomic, strong) NSString *herUrl;

@property (nonatomic, strong) NSString *my;
@property (nonatomic, strong) NSString *her;
...
- (void)viewDidLoad {
    ...
    [self getImg];
    _my = [self getImgHaHa];
    _her = [self getImgHaHa];
}

-(void) getImg{
    AVQuery *query = [AVFile query];
    [query whereKey:@"name" equalTo:[AVUser currentUser].objectId];
    [query orderByDescending:@"createdAt"];
    AVFile *file = [AVFile fileWithAVObject:[query getFirstObject]];
    self.myUrl = file.url;
    AVQuery *query1 = [AVFile query];
    [query1 whereKey:@"name" equalTo:self.otherChater.objectId];
    [query1 orderByDescending:@"createdAt"];
    AVFile *file1 = [AVFile fileWithAVObject:[query1 getFirstObject]];
    self.herUrl = file1.url;
...
//        cell.message = message;
        ...
        if (_myUrl) {
            [cell.myImg sd_setImageWithURL:[NSURL URLWithString:_myUrl]];
        }else{
            cell.myImg.image = [UIImage imageNamed:_my];
        }
        return cell;
    }else{
...
//        cell.message =message;
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        if (_herUrl) {
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:_herUrl]];
        }else {
            cell.imgView.image = [UIImage imageNamed:_her];
        }
        return cell;
    }
}
