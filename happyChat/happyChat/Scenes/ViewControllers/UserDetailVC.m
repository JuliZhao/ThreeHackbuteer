...
-(void) getImage{
    ...
    [query whereKey:@"userId" equalTo:self.likeFriend.objectId];
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        if (!error) {
            AVObject *obj = objects.lastObject;
//            AVFile *userAV = [obj objectForKey:@"userHeadImg"];
//            NSData *userImgData = [userAV getData];
//            if (userImgData) {
//                self.userImgView.image = [UIImage imageWithData:userImgData];
//            }
            AVQuery *img = [AVFile query];
            [img whereKey:@"name" equalTo:@"%@.png"];
            [img orderByDescending:@"createdAt"];
            AVFile *imgFile = [AVFile fileWithAVObject:[img getFirstObject]];
            if (imgFile) {
                [self.userImgView sd_setImageWithURL:[NSURL URLWithString:imgFile.url]];
            }else{
                self.userImgView.image = [UIImage imageNamed:[self getImg]];
            }
            NSString *str = [obj objectForKey:@"userSignature"];
            if (str) {
                self.userSignatureLab.text = str;
            }
        }
    }];
}
